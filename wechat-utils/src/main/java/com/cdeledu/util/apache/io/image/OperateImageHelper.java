package com.cdeledu.util.apache.io.image;
 ...
 * @类描述: 图片相关的操作
 * @创建者: 皇族灬战狼
 * @创建时间: 2017年4月7日 下午5:57:33
 * @版本: V1.0
 * @since: JDK 1.7
 */
public final class OperateImageHelper {
	/** ----------------------------------------------------- Fields start */
	// 图形交换格式
	...
	public static BufferedImage Rotate(Image src, int angel) {
		...
		Rectangle rect_des = CalcRotatedSize(new Rectangle(new Dimension(src_width, src_height)),
				angel);
	 ...
	 */
	public static boolean cutImage(InputStream srcImageFile, File dest, int x, int y, int w,
			int h) {
		try {
	 ...
	 * @return
	 */
	public static boolean cutAndRotateImage(InputStream srcImageFile, File dest, int x, int y,
			int w, int h, int r) {
		try {
			/*
	 ...
	 */
	public static boolean scale2(String srcImageFile, String result, int height, int width,
			boolean bb) {
		try {
	...
	public static boolean pressImage(String pressImg, String srcImageFile, String destImageFile,
		...
		try {
			...
			g.drawImage(src_biao, (width - wideth_biao - x), (height - height_biao - y),
	 ...
	 *            透明度：alpha 必须是范围 [0.0, 1.0] 之内（包含边界值）的一个浮点数字
	 */
	public static boolean pressText(String pressText, String srcImageFile, String destImageFile,
